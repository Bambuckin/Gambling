...
|---|---|---|---|
| `apps/web` | Web runtime shell, login/user/admin/denied routes, middleware pre-filter, and route-level access entry flow (`src/lib/access/*`) | Terminal automation details, ledger mutation logic, queue worker behavior, direct auth-store mutation bypassing application service | `apps/web/src/app/layout.tsx`, `apps/web/src/app/page.tsx`, `apps/web/src/app/login/page.tsx`, `apps/web/src/app/lottery/[lotteryCode]/page.tsx`, `apps/web/src/app/admin/page.tsx`, `apps/web/src/middleware.ts` |
