...
|---|---|---|---|
| `apps/web` | Web runtime shell, login/user/admin/denied/debug routes, middleware pre-filter, route-level access flow (`src/lib/access/*`), registry runtime composition (`src/lib/registry/*`), admin registry mutation flow (`src/lib/registry/admin-registry.ts`), draw runtime composition (`src/lib/draw/*`), ledger runtime composition (`src/lib/ledger/*`), purchase request + admin queue runtime composition (`src/lib/purchase/*`), ticket read runtime composition (`src/lib/ticket/*`), terminal health runtime composition (`src/lib/terminal/*`), observability runtime composition (`src/lib/observability/*`), and dynamic lottery form renderer (`src/lib/lottery-form/*`) | Terminal automation details, ledger mutation rules, queue worker behavior, direct auth-store mutation bypassing application service, registry business rules embedded in route files | `apps/web/src/app/layout.tsx`, `apps/web/src/app/page.tsx`, `apps/web/src/app/login/page.tsx`, `apps/web/src/app/lottery/[lotteryCode]/page.tsx`, `apps/web/src/app/admin/page.tsx`, `apps/web/src/app/debug/access-lab/page.tsx`, `apps/web/src/app/debug/admin-ops-lab/page.tsx`, `apps/web/src/app/debug/registry-lab/page.tsx`, `apps/web/src/app/debug/wallet-lab/page.tsx`, `apps/web/src/app/debug/purchase-lab/page.tsx`, `apps/web/src/app/debug/terminal-lab/page.tsx`, `apps/web/src/app/debug/ticket-lab/page.tsx`, `apps/web/src/middleware.ts` |
| `apps/terminal-worker` | Worker process boot, queue reservation polling, execution lock lifecycle, deterministic handler resolution, and forwarding attempt outcomes to application services | UI rendering, user session logic, lottery pricing/domain rules, direct queue/request/ticket store mutations | `apps/terminal-worker/src/main.ts`, `apps/terminal-worker/src/lib/terminal-handler-runtime.ts` |
...
| `packages/application` | Typed ports and use cases for terminal execution, queue, terminal execution lock, deterministic handler binding registry, attempt-result journaling, ticket persistence, ticket verification queue orchestration, ticket verification result normalization, ticket read/query projections, winnings credit orchestration, retry policy orchestration, terminal health projection, admin queue snapshot + priority orchestration, admin operations snapshot projection, operations audit emission, operations alert aggregation, time source, access/session orchestration, wallet ledger orchestration, access audit emission, lottery registry orchestration (including enable/disable/reorder mutations), draw freshness orchestration, purchase draft validation/quote orchestration, purchase request snapshot orchestration, reserve+queue purchase orchestration, queue reservation for execution, and purchase status query projections | Concrete adapter SDK bindings, JSX/UI concerns, persistence schema definitions | `packages/application/src/index.ts`, `packages/application/src/services/access-service.ts`, `packages/application/src/services/wallet-ledger-service.ts`, `packages/application/src/services/lottery-registry-service.ts`, `packages/application/src/services/draw-refresh-service.ts`, `packages/application/src/services/purchase-draft-service.ts`, `packages/application/src/services/purchase-request-service.ts`, `packages/application/src/services/purchase-orchestration-service.ts`, `packages/application/src/services/admin-queue-service.ts`, `packages/application/src/services/admin-operations-query-service.ts`, `packages/application/src/services/operations-audit-service.ts`, `packages/application/src/services/operations-alert-service.ts`, `packages/application/src/services/purchase-request-query-service.ts`, `packages/application/src/services/ticket-query-service.ts`, `packages/application/src/services/purchase-execution-queue-service.ts`, `packages/application/src/services/terminal-handler-resolver-service.ts`, `packages/application/src/services/terminal-execution-attempt-service.ts`, `packages/application/src/services/ticket-persistence-service.ts`, `packages/application/src/services/ticket-verification-queue-service.ts`, `packages/application/src/services/ticket-verification-result-service.ts`, `packages/application/src/services/terminal-retry-service.ts`, `packages/application/src/services/terminal-health-service.ts`, `packages/application/src/ports/access-audit-log.ts`, `packages/application/src/ports/operations-audit-log.ts`, `packages/application/src/ports/ledger-store.ts`, `packages/application/src/ports/lottery-registry-store.ts`, `packages/application/src/ports/draw-store.ts`, `packages/application/src/ports/purchase-request-store.ts`, `packages/application/src/ports/purchase-queue-store.ts`, `packages/application/src/ports/ticket-store.ts`, `packages/application/src/ports/ticket-verification-job-store.ts`, `packages/application/src/ports/terminal-execution-lock.ts`, `packages/application/src/ports/terminal-handler-registry.ts` |
| `packages/infrastructure` | Adapter implementations for infrastructure concerns, including in-memory and Postgres-backed access identity/session stores, password verifier, access audit log, operations audit log, lottery registry storage, draw snapshot storage, immutable ledger storage, purchase request storage, purchase queue storage, ticket storage, ticket verification job storage, and terminal execution lock | Domain state-machine ownership, handler business behavior, UI composition | `packages/infrastructure/src/index.ts`, `packages/infrastructure/src/access/in-memory-identity-store.ts`, `packages/infrastructure/src/access/in-memory-session-store.ts`, `packages/infrastructure/src/access/in-memory-access-audit-log.ts`, `packages/infrastructure/src/observability/in-memory-operations-audit-log.ts`, `packages/infrastructure/src/registry/in-memory-lottery-registry-store.ts`, `packages/infrastructure/src/draw/in-memory-draw-store.ts`, `packages/infrastructure/src/ledger/in-memory-ledger-store.ts`, `packages/infrastructure/src/purchase/in-memory-purchase-request-store.ts`, `packages/infrastructure/src/purchase/in-memory-purchase-queue-store.ts`, `packages/infrastructure/src/purchase/in-memory-ticket-store.ts`, `packages/infrastructure/src/purchase/in-memory-ticket-verification-job-store.ts`, `packages/infrastructure/src/purchase/in-memory-terminal-execution-lock.ts`, `packages/infrastructure/src/postgres/postgres-client.ts`, `packages/infrastructure/src/postgres/postgres-schema.ts`, `packages/infrastructure/src/postgres/postgres-access-store.ts`, `packages/infrastructure/src/postgres/postgres-registry-draw-store.ts`, `packages/infrastructure/src/postgres/postgres-ledger-store.ts`, `packages/infrastructure/src/postgres/postgres-purchase-store.ts`, `packages/infrastructure/src/postgres/postgres-operations-audit-log.ts` |
...
22. Retry-vs-final-error decision in worker flow must route through `TerminalRetryService` using domain retry policy rules.
23. `apps/web/src/app/debug/terminal-lab/page.tsx` is verification-only and may display terminal health projection, but must not expose queue/request mutation controls.
24. Successful purchase outcomes in `TerminalExecutionAttemptService` may persist tickets only through `TicketPersistenceService`, never via worker/runtime direct store writes.
25. Ticket verification queue lifecycle (`queued -> verifying -> done|error`) must be orchestrated through `TicketVerificationQueueService`, not runtime store mutations.
26. Ticket verification terminal results in worker flow must be normalized through `TicketVerificationResultService`; winnings credit must go through `WalletLedgerService.creditWinnings`.
27. `apps/web/src/app/debug/ticket-lab/page.tsx` is verification-only and may read ticket outcomes, but must not expose ticket/ledger mutation controls.
...
11. Hardcoding retry/final-error branching in worker runtime instead of application/domain retry policy.
12. Adding terminal queue/request mutation controls to `terminal-lab` debug route.
13. Writing ticket records directly from worker runtime or web routes without `TicketPersistenceService`.
14. Mutating ticket verification jobs directly in worker/runtime code instead of `TicketVerificationQueueService`.
...
- `corepack pnpm --filter @lottery/application test -- ticket-persistence-service` validates idempotent ticket persistence for successful request outcomes.
- `corepack pnpm --filter @lottery/application test -- ticket-verification-queue-service` validates enqueue/reserve/complete verification job orchestration.
- `corepack pnpm --filter @lottery/application test -- ticket-verification-result-service wallet-ledger-service` validates verification-result normalization and idempotent winnings credit.
- `corepack pnpm --filter @lottery/application test -- ticket-query-service` validates user-scoped ticket read projections and ordering.
- `corepack pnpm --filter @lottery/application test -- terminal-retry-service` validates retry exhaustion and terminal failure classification.
- `corepack pnpm --filter @lottery/application test -- terminal-health-service` validates `idle|busy|degraded|offline` state projection.
- `corepack pnpm --filter @lottery/application test -- admin-queue-service` validates admin queue snapshot projection and priority mutation flow boundaries.
- `corepack pnpm --filter @lottery/application test -- operations-audit-service` validates operations audit event structure and append ordering.
- `corepack pnpm --filter @lottery/application test -- operations-alert-service` validates operations alert aggregation from terminal/queue/finance signals.
- `corepack pnpm --filter @lottery/lottery-handlers typecheck` validates handler registry exports and type contracts.
- `corepack pnpm --filter @lottery/terminal-worker typecheck` validates worker queue reservation wiring, lock usage, and verification-result service integration.
